app.controller('ChatRoomController', [
    "$scope", "$timeout", "$filter", "ConsultationService", "AppointmentService", "$http",
    function ($scope, $timeout, $filter, ConsultationService, AppointmentService, $http) {
            ...
            recognition.onresult = function (event) {
                ...
                for (let i = event.resultIndex; i < event.results.length; ++i) {
                    ...
                    if (event.results[i].isFinal) {
                        accumulatedTranscript += transcriptPiece.trim() + " ";
                    } else {
                        interimTranscript += transcriptPiece;
                    }
                }

                // Show live transcript as it builds
                document.getElementById('liveTranscript').innerText = accumulatedTranscript + " " + interimTranscript;

                chat.server.broadcastTranscript($scope.appointmentID, accumulatedTranscript + " " + interimTranscript);
            };
        ...
        function saveTranscriptToServer(transcriptText) {
            if (!transcriptText || transcriptText.trim() === "") {
                console.warn("No transcript to save.");
                return;
            }

            $http.post('/Transcription/SaveTextTranscript', {
        ...
        chat.client.receiveTranscriptUpdate = function (transcript) {
        ...
        $scope.endCall = () => {
            ...
            stopLiveTranscription();
            stopRecording();
            saveTranscriptToServer(accumulatedTranscript);
